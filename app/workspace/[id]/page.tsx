...
export default function WorkspacePage() {
  ...
  // Handle timeframe changes - re-aggregate candles
  useEffect(() => {
  ...
  async function init() {
    ...
    // Load real XAUUSD CSV data
    let c: Candle[]
    try {
      c = await loadXauUsdData()
    } catch {
      // Fallback: try candle_cache from DB
    ...
    // Filter candles to the session's selected date range
  ...
  const advance = useCallback(async (steps: number) => {
    ...
    for (const candle of slice) {
      updatedTrades = updatedTrades.map(trade => {
        ...
        if (exitPrice !== null) {
          const pnl = calcPnl(trade.side, trade.entry_price, exitPrice, trade.quantity)
          deltaBal += pnl
  ...
  async function closeTrade(tradeId: string) {
    ...
    if (!cur || !trade) return
    const pnl    = calcPnl(trade.side, trade.entry_price, cur.close, trade.quantity)
  ...
  const handleSetSL = async (tradeId: string, price: number) => {
    await supabase.from('trades').update({ stop_loss: price }).eq('id', tradeId)
    setTrades(prev => prev.map(t => t.id === tradeId ? { ...t, stop_loss: price } : t))
  }

  const handleSetTP = async (tradeId: string, price: number) => {
    await supabase.from('trades').update({ take_profit: price }).eq('id', tradeId)
    setTrades(prev => prev.map(t => t.id === tradeId ? { ...t, take_profit: price } : t))
  }
  ...
  const openPnl  = cur
    ? openTr.reduce((a, tr) => a + calcPnl(tr.side, tr.entry_price, cur.close, tr.quantity), 0)
    : 0
  ...
  const getFormattedDate = (timestamp: number, tzValue: string) => {
    ...
    const tzDate = new Date(utcTime + tzInfo.offset * 60 * 60 * 1000)
  ...
  const dateStr  = (() => {
    // Show actual candle timestamp from CSV
    if (cur?.time) {
      return getFormattedDate(cur.time, timezone)
    }
    return interpolateDate(session.start_date, session.end_date, idx, candles.length)
  })()
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', minWidth: 0, minHeight: 0 }}>
          ...
          <div className="flex items-center gap-3 px-4 py-2.5 shrink-0 flex-wrap"
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => advance(skipVal)} disabled={done}
              ...
              {tradeTab === 'open' ? (
                openTr.length === 0 ? (
                  ...
                  <table className="w-full">
                    ...
                    <tbody>
                      {openTr.map(tr => {
                        const upnl = cur ? calcPnl(tr.side, tr.entry_price, cur.close, tr.quantity) : 0
                        return (
